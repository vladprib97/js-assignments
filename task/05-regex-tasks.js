...
function getRegexForGuid() {
    var regular = /^{[\da-fA-F]{8}-([\da-fA-F]{4}-){3}[\da-fA-F]{12}}$/;
...
function getRegexForIPv4() {
    var regular = /^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
    return new RegExp(regular);
}
...
function getRegexForSSN() {
    var regular = /^(?!0{3})\d{3}-(?!0{2})\d{2}-(?!0{4})\d{4}$/;
    return new RegExp(regular);
}
...
function getPasswordValidator(minLength) {
    var regular = "(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9]{" + minLength + ",}$";
    return new RegExp(regular);
}
